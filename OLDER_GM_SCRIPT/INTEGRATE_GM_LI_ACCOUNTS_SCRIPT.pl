#!/usr/bin/perl
## Version 3.1 Updated on 05-March-2014 ---->
use Cwd;
use WWW::Mechanize;
...
while (my $filename = readdir(DIR)) {
	...
	if($checkSearch ne ""){
		if($filename =~ /$checkSearch/is){next;}
	}
	...
	$contArray=();
	DELETE_LI();
	print "------ Completed the Script to ADD the Contacts into Gmail ---------\n";
	print "DONE\n";
}
closedir(DIR);
unlink($resumeFile);
...
sub GET_DATA
{
	...
	for($i=0;$i<$total;$i++)
	{
		...
		print "Now sleeping for 62 secs\n";
		sleep(62); ## sleep for 62 seconds and then get all records, at every alphabet. (was 5 secs)
	}
}
...
sub DELETE_LI
{
	...
	for($i=0;$i<$total;$i++)
	{
		...
		print "Records -> ".$#contArray."\n";
		sleep(15); ## 15 sec sleep to get all records at every alphabet.
	}
	DELETE();
}
	...
	for($i=1;$i<=$total;$i++){
		...
		if($i%20 eq 0 || ($total-$i)<20){
			...
			if($htmlPage =~ m/id\W+contacts-browser-form\W+(.*?)<\/form/is){
				...
				$agent->post('http://www.linkedin.com/people/submit-contact',\%formData);
				$htmlPage2 = $agent->content;
				writeToFile('output/8.htm',$htmlPage2);
				if($htmlPage2 =~ /<strong>\s*\d+\s*contact\(s\)\s+were\s+deleted\s+from\s+your\s+Contacts\s+list/is){
					print "Contacts Deleted ::: $i from a Total of $total ::: contID: $contID\n";
					writeToFile('output/8-1.htm',$htmlPage2);
				}elsif($htmlPage2 =~ /<h3>\s*Contact\s+Removal\s+Processing/is){
					print "Contacts Deleted : $i from a total of $total : contID: $contID\n";
					writeToFile('output/8-2.htm',$htmlPage2);
				}
			}else{
				writeToFile('output/manage_contact.htm',$htmlPage);
				ERROR("---Delete Form Not Found---!");
			}
			$contID='';
			print "sleep for 15 secs; contID: $contID\n";
			sleep(15); ## 15 sec sleep to delete every batch
		}
	}
	$contArray=();
	CHECK_CONTACTS();
...
sub CHECK_AND_DELETE
{
	...
	$agent->get("http://www.linkedin.com/connections?type=combined&trk=nav_responsive_sub_nav_network");
	$htmlPage = $agent->content;
	writeToFile('output/rand21.htm',$htmlPage);
	sleep(3);
	$agent->get("http://www.linkedin.com/company/home?trk=nav_responsive_sub_nav_companies");
	$htmlPage = $agent->content;
	writeToFile('output/rand22.htm',$htmlPage);
	sleep(3);
	$agent->get("https://www.linkedin.com/fetch/importAndInviteEntry?trk=nav_responsive_sub_nav_add_connections");
	$htmlPage = $agent->content;
	writeToFile('output/12.htm',$htmlPage);
	print "Going to Delete Contacts...\n";
	my $url="http://www.linkedin.com/people/contacts?sortAction=lastName&showInvited=true&membersOnly=false&trk=ac-manage-contacts";
	$agent->add_header();
	$agent->get($url);
	$htmlPage = $agent->content;
	writeToFile('output/13.htm',$htmlPage);
	$contID='';
	$total=$#contArrayDelete+1;
	for($i=1;$i<=$total;$i++){
		$contID.=" ".$contArrayDelete[$i];
		if($i%20 eq 0 || ($total-$i)<20){
			$contID=~s/^\s+|\s+$//is;
			my %formData=();
			if($htmlPage =~ m/id\W+contacts-browser-form\W+(.*?)<\/form/is){
				...
				$agent->post('http://www.linkedin.com/people/submit-contact',\%formData);
				$htmlPage2 = $agent->content;
				writeToFile('output/12-1.htm',$htmlPage2);
				if($htmlPage2 =~ /<strong>\s*\d+\s*contact\(s\)\s+were\s+deleted\s+from\s+your\s+Contacts\s+list/is){
					print "20 Contacts Deleted !\n";
					writeToFile('output/12-2.htm',$htmlPage2);
				}elsif($htmlPage2 =~ /<h3>\s*Contact\s+Removal\s+Processing/is){
					print "20 or more Contacts Deleted !\n";
					writeToFile('output/12-3.htm',$htmlPage2);
				}
			}
			$contID='';
		}
		sleep(2); ## 2 sec sleep to delete every batch
	}
}
